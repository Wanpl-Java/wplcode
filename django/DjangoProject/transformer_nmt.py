...
def translate(userInput):
    # 先修改用户的输入
    userInput = "BOS " + userInput + " EOS"
    user_input_list = []
    for i in userInput.split(" "):
        user_input_list.append(data.en_word_dict[i])
    # 将当前以单词id表示的英文语句数据转为tensor，并放入DEVICE中
    src = torch.from_numpy(np.array(user_input_list)).long().to(DEVICE)
    # 增加一维
    src = src.unsqueeze(0)
    # 设置attention mask
...
def evaluate(data, model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(data.dev_en)):
            print(data.dev_en[i]) # [2, 168, 201, 4, 3]
            # 打印待翻译的英文语句
            en_sent = " ".join([data.en_index_dict[w] for w in data.dev_en[i]])
            print("e!n:" + en_sent)
            print("e!n.len:%d" % len(en_sent))
            # print("\n" + en_sent)

            # 打印对应的中文语句答案
            cn_sent = " ".join([data.cn_index_dict[w] for w in data.dev_cn[i]])
            print("c!n:" + cn_sent)
            print("c!n.len:%d" % len(cn_sent))
            # print("".join(cn_sent))
...
class Translate:
    ...
    def translate_it(self, userInput):
...
# ts = Translate("like")
# print(ts.translate_it(ts.userInput))
'''# 开始预测
